...
public class ClientManagerAI : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        transforms.Add(new Trans(new Vector3(0, 1.74f, -1.9f), Quaternion.identity, Constants.Eye));
        transforms.Add(new Trans(new Vector3(-0.55f , 1f, -1.9f), Quaternion.identity, Constants.LeftHand));
        transforms.Add(new Trans(agent.position, agent.rotation, Constants.RightHand));
        ...
        if(ballTrans != null)
        {
            ball.MovePosition(ballTrans.Pos);
            ball.MoveRotation(ballTrans.Rot);
        } else
        {
            ball.MovePosition(Vector3.zero);
            ball.MoveRotation(Quaternion.identity);
        }
    }
