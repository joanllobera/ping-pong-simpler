...
public class FingerIKs : MonoBehaviour {
	...
	void Awake () {
        ...
        for (int i = 0; i < _handMalcomPoints.Length; ++i) {
            _handMalcomPointsRegister[i] = _handMalcomPoints[i].position;
        }
    ...
    void Update () {
        if (_leftHand.isLeft) {
            if (_leftHand.Points[0].activeSelf) {
                ...
                _handMalcomPoints[0].rotation = _leftHand.Points[0].transform.rotation;
            } else {
            ...
            for (int i = 1; i < _leftHand.Points.Count; ++i) {
                if (_leftHand.Points[i].activeSelf) {
                    _handMalcomPoints[i].position = _leftHand.Points[i].transform.position;
                } else { _handMalcomPoints[i].position = _handMalcomPointsRegister[i]; }
            }
        }
	}
}
